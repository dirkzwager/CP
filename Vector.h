...
public:
  ...
  void insertElement(const int& i, const T& el);
...
template <class T>
void Vector<T>::removeElement(const int& idx) {
  if (idx < 0 || idx >= Dim()) {
...
template <class T>
void Vector<T>::insertElement(const int& idx, const T& el) {
if (idx < 0 || idx > Dim()) {
    cout << "Warning, invalid index" << endl;
    return;
  }
